  ...
  return (
    ...
    <header className={classNames(s.header)}>
        <div className="pageWrapper">
            ...
            <li><Link to="/" className='text-bodyCopy text-m'>Home</Link></li>
            {
                data.allStrapiCategory.nodes.map(node => (
                <li key={node.id} >
                    <Link to={`/${node.slug}`} className='text-bodyCopy text-m'>{node.name}</Link>
                </li>
                )
                )
            }
            <li>
                <a className='button ml-6'  href='#' >Book appointment</a>
            </li>
            </ul>
        </div>
      </header>
