...
function ArticleListCard({item}) {
  return (
    <div className={classNames(style.ArticleListCard)}>
        ...
        <div className={classNames(style.ArticleListCardDec)}>
          <div className={classNames(style.ArticleListCardTag)}>{item.category}</div>
          <h4>{item.title}</h4>
          <aside dangerouslySetInnerHTML={{__html: item.description?.data?.childMarkdownRemark?.html}} />
          <div className={classNames(style.ArticleListCardFtr)}>
              <Link to={item.slug}>{item.button_label} 
              ...
              <span className={classNames(style.ArticleListCardDate)}>{item.publishedAt}</span>
          </div>
        </div>
    </div>
  )
